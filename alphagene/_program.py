...
class _Program(object):
    ...
    def execute(self, X):
        ...
        if isinstance(node, float):
            return pd.DataFrame(1, index=X[self.feature_names[0]].index,
                                columns=X[self.feature_names[0]].columns)
        if isinstance(node, int):
            return X[self.feature_names[node]]
        ...
        for node in self.program:
            ...
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:
                ...
                terminals = [pd.DataFrame(t, index=X[self.feature_names[0]].index,
                                          columns=X[self.feature_names[0]].columns) if isinstance(t, float)
                             else X[self.feature_names[t]] if isinstance(t, int)
    ...
    def fitness(self, parsimony_coefficient=None):
        ...
        for node in self.program:
            if isinstance(node, _Function):
                # Find all integers in node.name and convert them to int
                numbers = re.findall(r'\d+', node.name)
                for number in numbers:
                    ts_para += int(number)

        penalty = parsimony_coefficient * (len(self.program) + ts_para) * self.metric.sign
