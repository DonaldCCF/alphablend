...
def make_function(*, function, name, arity, wrap=True):
    ...
    # Check output shape
    args = [pd.DataFrame(np.ones((10, 10))) for _ in range(arity)]
    ...
    if not hasattr(function(*args), 'shape'):
        raise ValueError('supplied function %s does not return a numpy array.'
                         % name)
    if function(*args).shape != (10, 10):
        raise ValueError('supplied function %s does not return same shape as '
                         'input vectors.' % name)

    # # Check closure for zero & negative input arguments
    # args = [pd.DataFrame(np.zeros((10, 10))) for _ in range(arity)]
...
def function_wrapper(func, name, arity):
    """
    Wraps the given function with make_function, setting the specified name and arity.
    """
    return make_function(function=func, name=name, arity=arity)
